...
;; todo: make square v triangle option
(define (binary-biased-cons/e e1 n1 e2 n2)
  (define s1 (enum-count e1))
  (define s2 (enum-count e2))
  (when (not (and (infinite? s1) (infinite? s2)))
    (error 'binary-biased-cons/e "only works on inf enums for now"))
  (define (from z)
    (define-values (q r) (integer-root/remainder z (n1 . + . n2)))
    (define q^n1   (expt q n1))
    (define q+1^n1 (expt (+ q 1) n1))
    (define q^n2   (expt q n2))
    (define switch-point (* (q+1^n1 . - . q^n1) q^n2))
    (define-values
      (basex basey diffx diffy)
      (cond [(r . < . switch-point)
             (define basex q^n1)
             (define basey 0)
             (define-values (diffx diffy) (quotient/remainder r q^n2))
             (values basex basey diffx diffy)
            ]
           [else
            (define basex 0)
            (define basey q^n2)
            (define-values (diffx diffy)
  ...
  (define (to xy)
    ...
    (define-values (xq xr) (integer-root/remainder i n1))
    (define-values (yq yr) (integer-root/remainder j n2))
    (define q (max xq yq))
    (cond [(xq . > . yq)
           ; (define basex xq)
           ; (define basey 0)
           ;; r % (q^n2) = (i - q^xq) rem j
           ;; r = (i-q^xq)(q^n2) + j
           (define layer-offset (expt q (n1 . + . n2)))
           (+ layer-offset
              (* xr (expt q n2))
              j)]
          [else
           (define offset (+ (* (expt (+ q 1) n1)
                                (expt q n2))))
           ;; (r - offset) % ((q+1)^n2 - q^n2) = i rem (j - q^yq)
           ;; r = offset + (i * ((q+1)^n2 - q^n2)) + j - q^yq
           (+ offset
              (* i
                 (- (expt (+ q 1) n2)
                    (expt q       n2)))
